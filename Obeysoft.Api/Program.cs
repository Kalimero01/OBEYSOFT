...
// Statik dosyalar (upload edilen medya için)
app.UseStaticFiles();
...
using (var scope = app.Services.CreateScope())
{
    ...
    try
    {
        ...
        if (automigrate)
        {
            ...
            catch (Npgsql.PostgresException pex) when (recreateOnModelChange && (pex.SqlState == "42P07" || pex.SqlState == "42710"))
            {
                logger.LogWarning(pex, "Schema drift detected. Dropping and recreating database (Development only).");
